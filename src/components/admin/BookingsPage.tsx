...
const STATUS_COLOR: Record<string, string> = { confirmed: 'var(--blue)', pending: 'var(--ora)', cancelled: 'var(--red,#ef4444)', seated: 'var(--grn)' }

const EMPTY: Booking = { id: '', name: '', phone: '', date: '', time: '', partySize: 2, module: 'restaurant', note: '', status: 'confirmed' }

export default function BookingsPage() {
  const [bookings, setBookings] = useState<Booking[]>(() => storage.get<Booking[]>('bookings') ?? [])
  ...
  const save = () => {
    ...
    const updated = isNew ? [modal, ...bookings] : bookings.map(b => b.id === modal.id ? modal : b)
    persist(updated)
    setModal(null)
  }
  ...
  const set = (patch: Partial<Booking>) => setModal(m => m ? { ...m, ...patch } : m)
    ...
    <div style={{ padding: '18px 20px', overflowY: 'auto', height: '100%', flex: 1 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--txt3)', marginTop: 3 }}>{todayCount} today · {upcomingCount} upcoming</div>
        </div>
        <button onClick={openAdd} style={{ padding: '9px 18px', borderRadius: 'var(--r)', background: 'var(--blue)', color: '#fff', border: 'none', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+ New Booking</button>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div key={b.id} style={{ padding: '13px 16px', borderBottom: '1px solid var(--bdr2)', display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              <button onClick={() => del(b.id)} style={{ padding: '5px 10px', borderRadius: 6, background: 'transparent', border: '1px solid var(--bdr)', color: 'var(--red,#ef4444)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>Del</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div onClick={() => setModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg2)', border: '1px solid var(--bdr)', borderRadius: 'var(--r4)', width: '100%', maxWidth: 420, padding: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              <div style={{ gridColumn: '1 / -1' }}><label style={lbl}>Guest Name</label><input style={inp} value={modal.name} onChange={e => set({ name: e.target.value })} /></div>
              <div><label style={lbl}>Phone</label><input style={inp} value={modal.phone} onChange={e => set({ phone: e.target.value })} /></div>
              <div><label style={lbl}>Party Size</label><input type="number" min={1} max={100} style={inp} value={modal.partySize} onChange={e => set({ partySize: Number(e.target.value) })} /></div>
              <div><label style={lbl}>Date</label><input type="date" style={inp} value={modal.date} onChange={e => set({ date: e.target.value })} /></div>
              <div><label style={lbl}>Time</label><input type="time" style={inp} value={modal.time} onChange={e => set({ time: e.target.value })} /></div>
              <div>
                <label style={lbl}>Module</label>
                <select style={inp} value={modal.module} onChange={e => set({ module: e.target.value as Booking['module'] })}>
              ...
              <div>
                ...
                <select style={inp} value={modal.status} onChange={e => set({ status: e.target.value as Booking['status'] })}>
                  <option value="confirmed">Confirmed</option>
                  <option value="pending">Pending</option>
                  <option value="seated">Seated</option>
                  <option value="cancelled">Cancelled</option>
              ...
              <div style={{ gridColumn: '1 / -1' }}><label style={lbl}>Note</label><input style={inp} value={modal.note} onChange={e => set({ note: e.target.value })} placeholder="Special requests, allergies…" /></div>
            </div>
