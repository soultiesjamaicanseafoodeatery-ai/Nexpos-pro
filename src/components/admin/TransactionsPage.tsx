...
const DATE_LABELS: Record<string, string> = {
  all: 'All Time', today: 'Today', yesterday: 'Yesterday', week: 'Last 7 Days', month: 'This Month',
}
...
export default function TransactionsPage() {
  ...
  const fetchTxs = useCallback(async () => {
    ...
    try {
      ...
      console.log('[NexPOS:TransactionsPage] source=Supabase | count=' + txs.length)
      setTxList(txs)
    } catch (err) {
  ...
  useEffect(() => { fetchTxs() }, []) // eslint-disable-line
  ...
  useEffect(() => { setPage(1) }, [search, filter, dateFilter])
    ...
    <div className="adm" style={{ padding: '18px 20px', overflowY: 'auto', height: '100%', flex: 1 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--txt3)', marginTop: 3 }}>
            {loading
              ...
              : `${filtered.length} records · ${sym}${totalRev.toFixed(2)} total · ${DATE_LABELS[dateFilter]}${filter !== 'all' ? ` · ${filter}` : ''}`}
          </div>
        </div>
      ...
      <div style={{ background: 'var(--surf)', border: '1px solid var(--bdr)', borderRadius: 'var(--r3)', overflow: 'hidden', marginBottom: 13 }}>
        ...
        <div style={{ padding: '9px 12px', borderBottom: '1px solid var(--bdr)', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {(['all','restaurant','bar','carwash'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`btn btn-sm ${filter === f ? 'btn-pr' : 'btn-gh'}`}>
              {f === 'all' ? 'All Modules' : `${MOD_ICON[f]} ${f.charAt(0).toUpperCase()+f.slice(1)}`}
            </button>
          ))}
        </div>
        {/* Row 2: date filter */}
        <div style={{ padding: '7px 12px', borderBottom: '1px solid var(--bdr)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: 'var(--txt3)', marginRight: 2 }}>Date:</span>
          {(['all','today','yesterday','week','month'] as const).map(d => (
            <button key={d} onClick={() => setDateFilter(d)} className={`btn btn-sm ${dateFilter === d ? 'btn-pr' : 'btn-gh'}`}>
              {DATE_LABELS[d]}
            </button>
          ))}
        </div>
