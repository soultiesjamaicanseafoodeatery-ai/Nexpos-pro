...
export default function POSPage() {
  ...
  // moduleData comes from seed directly in AppShell context
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { MODULE_DATA } = require('@/lib/data/seed')
  ...
  const toggleItem = (item: MenuItem) => {
    if (ps.selItem?.id === item.id) {
      setPOS({ selItem: null, selAddons: [] })
    } else {
      setPOS({ selItem: item, selAddons: [], qty: 1 })
    }
  }
  ...
  const checkout = () => {
    ...
    const tx: Transaction = {
      id: Date.now(),
      ts: new Date().toLocaleDateString('en-US', { month:'2-digit', day:'2-digit' }) + ' ' + new Date().toLocaleTimeString('en-US', { hour:'2-digit', minute:'2-digit' }),
      mod: activeModule,
      cashier: currentUser.name,
      userId: currentUser.id,
      customer: ps.selTable ? `Table ${ps.selTable}` : ps.customerName || 'Walk-in',
    ...
    // Reset POS
    setPOS({ selItem: null, selAddons: [], qty: 1, note: '', selTable: null, customerName: '' })
  }

  const selCls = { restaurant: 's-o', bar: 's-p', carwash: 's-b' }[activeModule]
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 268px 308px', flex: 1, overflow: 'hidden' }}>
              ...
              return (
                <div key={item.id} onClick={() => toggleItem(item)} style={{
                  ...
                  <div style={{ height: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0, overflow: 'hidden' }}>
                    ...
                    {selected && (
                      <div style={{ position: 'absolute', top: 10, right: 10, width: 26, height: 26, borderRadius: '50%', background: mod.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff', zIndex: 10, boxShadow: '0 2px 8px rgba(0,0,0,.4)' }}>✓</div>
                    )}
                    {item.duration && (
                  ...
                  <div style={{ padding: '10px 11px 11px', flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
                      <span style={{ fontSize: 20, fontWeight: 800, fontFamily: 'var(--mono)', letterSpacing: '-.5px', color: item.accent ?? mod.color }}>{sym}{item.price}</span>
                    </div>
                  </div>
                </div>
              )
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid var(--bdr)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
          {mod.addons.filter((a: Addon) => a.active).map((addon: Addon) => {
            ...
            const aoCls = { restaurant: 'o', bar: 'p', carwash: 'g' }[activeModule]
            return (
              <div key={addon.id} onClick={() => ps.selItem && toggleAddon(addon)} style={{
                ...
                <div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: addon.price === 0 ? 'var(--grn)' : 'var(--txt)' }}>{addon.price === 0 ? 'Free' : `+${sym}${addon.price}`}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
          {!ps.selItem ? (
            ...
            <div style={{ background: 'var(--surf)', borderRadius: 'var(--r)', padding: '11px 12px', marginBottom: 7, display: 'flex', gap: 9 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--txt)' }}>{ps.selItem.name} {ps.qty > 1 && <span style={{ color: 'var(--txt3)' }}>×{ps.qty}</span>}</div>
                <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 2 }}>{sym}{ps.selItem.price} each</div>
                {ps.selAddons.map(a => (
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 0', borderBottom: '1px solid var(--bdr)', fontSize: 12 }}>
                    ...
                    <span style={{ fontFamily: 'var(--mono)', color: 'var(--txt3)' }}>+{sym}{a.price}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid var(--bdr)', background: 'var(--bg3)', flexShrink: 0 }}>
          ...
          ].filter(Boolean).map((row: { label: string; value: string } | false, i) => row && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 6, margin: '10px 0' }}>
            {[['cash','💵','Cash'],['card','💳','Card'],['tab','📑','Tab'],['qr','📱','QR Pay']].map(([key, ic, lbl]) => (
              <button key={key} onClick={() => setPOS({ payMethod: key })} style={{
                padding: '13px 4px', borderRadius: 'var(--r)', border: `2px solid ${ps.payMethod === key ? 'var(--blue)' : 'var(--bdr2)'}`,
