...
export default function CloseShiftWizard() {
  ...
  const totalTax   = shiftTxs.reduce((s,tx)=>s+((tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.gct??tx.tax??0):0), 0)
  const totalGrat  = shiftTxs.reduce((s,tx)=>s+((tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.gratuity??0):0), 0)
  ...
  shiftTxs.forEach(tx => {
    ...
    empMap[n].count++; empMap[n].total += tx.total; empMap[n].tips += (tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.gratuity??0):0
  })
  ...
  const renderSales = () => {
    ...
    shiftTxs.filter(tx => tx.mod !== 'carwash').forEach(tx => {
      ...
      modStats[m].count++
      modStats[m].sub   += (tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.sub??tx.total):tx.total
      modStats[m].disc  += tx.disc  ?? 0
      modStats[m].tax   += (tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.gct??tx.tax??0):0
      modStats[m].grat  += (tx.orderType==='dine-in' && typeof tx.ts==='string' && tx.ts.includes('T'))?(tx.gratuity??0):0
